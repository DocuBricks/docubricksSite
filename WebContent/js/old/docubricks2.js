...
function getBricksMap(db) {	
	var ret={};
	pforeach(db["brick"],function(brick){  //media disappears here!
		ret[brick.id]=brick;
//		console.log(brick);
	});
	return ret;
...
function addBrick(dx, thisbrick, db){
	...
	var form2 = $("#brickcontent").get(0).cloneNode(true);
	console.log(form2);
	dx.appendChild(form2);
	form2=$(form2);
	var brickbody=form2.get(0);//$(form2).find("#brickcontent");
...
//	qj1.setAttribute("class","row");
	...
	if(thisbrick.media != undefined){
		console.log(thisbrick.media);
		var toi=thisbrick.media[0];
		var qj1b=document.createElement("div");
		qj1b.setAttribute("class","col5 colExample");
		var img=document.createElement("img");
		if(toi == "undefined"){
							//dynamically add an image and set its attribute
			img.setAttribute("src",docubricksfilebase + 'images/Logo.png');
								//img.id="picture"
		}else{
			img.setAttribute("src",docubricksfilebase + thisbrick.media[0]);
							//img.id="picture"
		}
	...
	addsomehow(brickbody, thisbrick, "notes", "Notes: ");
	...
	if(anyLegal)
		brickbody.appendChild(legalnode);
	...
	*/
	...
	addInstruction(brickbody, thisbrick, thisbrick.assembly_instruction, db);

	
	////////////////////////////////////////////////////////////////////////
	// BOM
	addBrickBOM(brickbody, thisbrick, db);
	...
	if (instruction.step.length==0){
		...
		for(var muj=0;muj<instruction.step.length;muj++){
			...
			if("media" in thisstep && "file" in thisstep.media && "url" in thisstep.media.file){
				stimgsrc=docubricksfilebase + thisstep.media.file.url;
				var img=document.createElement("img");
