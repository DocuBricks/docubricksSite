...
var docubricksfilebase="./";   //may need a way to modify this!


// highest quality version https://github.com/mahogny/DBViewer/blob/master/docubricks_koruza/docubricks.js
		...
		if(id===db2){
			...
			if (mu.children('file').length>0){
				...
				if(mu.children('file').length===1){
					...
					bimg.setAttribute("src",docubricksfilebase + brickimgsrc);
					...
					bimg.setAttribute("src",docubricksfilebase + brimgsrc);
					bimg.setAttribute("width","100%");
					...
					gmenu.style.marginLeft = "0px"
						gmenu.style.marginRight = "0px"
							...
							for (var j=0;j<mu.children('file').length;j++){
								...
								gimgj.setAttribute("src",docubricksfilebase + $muj.getAttribute('url'));
				...
				$( this ).find('step').each(function() {
					...
					if (mu.children('file').length>0){
						if(mu.children('file').length===1){
							...
							bimgx.setAttribute("src",docubricksfilebase + brimgsrcx);
							...
							gmenux.style.marginLeft = "0px"
								gmenux.style.marginRight = "0px"
									for (var j=0;j<mu.children('file').length;j++){
										...
										gimgjx.setAttribute("src",docubricksfilebase + $mujx.getAttribute('url'));
...
/**
 * Render total bill of materials
 */
function renderBOM(db,dx){
	...
	var parts={};
	var count={};
	var xdb=XML2jsobj(db);
	partcount = getPhysicalPartCount(xdb);
	db.find('physical_part').each(function () {
		var id = $(this).attr('id');
		var name =$(this).children('description').text();
		parts[id]={};
		parts[id].name=name;
		parts[id].supplier=$(this).children('supplier').text();
		count[id]=0;
		//console.log(id);
		//console.log(name);
	...
	/*
	db.find('brick').each(function(bricki, bricke){
		$(bricke).find('function').each(function(fi, fe){
			var quant= $(fe).children('quantity').text();
			var id=$(fe).children('implementation').attr('id');
			// console.log(quant);
			// console.log(id);
			// console.log(parts[id]);
			// console.log(text0(parts[id]))
	...
	//For each BOM entry
	for(var pid in parts) { //use xdb
//	for (var i = 0; i < keys.length; i++) {
		var row = document.createElement("tr");
		var thispart=xdb.parts[pid];
		
		var cell1 = document.createElement("td");
		

		var cellText1 = document.createTextNode(thispart.name);
		var celltxta=document.createElement("a")
		celltxta.setAttribute("href","#"+pid);   //this might be wrong TODO
		celltxta.appendChild(cellText1);
		cell1.appendChild(celltxta);
		row.appendChild(cell1);
		/////////////////
		...
		var cell2 = document.createElement("td");
		var cellText2 = document.createTextNode(count[pid]);
		cell2.appendChild(cellText2);
		row.appendChild(cell2);

		var cell3 = document.createElement("td");
		var cellText3 = document.createTextNode(thispart.supplier);
		cell3.appendChild(cellText3);
		row.appendChild(cell3);
...
/****************************************/
function loadxmlXSLT(){
	//think this can be done less convoluted?
//	console.log("hello");
	var xmls = document.getElementById("hiddendata").children[0];
...
/** TODO: for the future. can request content by a call to the API */
function loadxmlSite(xmls){
	xmlDoc = $.parseXML( xmls )
	$xml = $( xmlDoc )
